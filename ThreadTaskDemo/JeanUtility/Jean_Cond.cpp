...
namespace JeanUtilityInternal
{
	...
	bool Semaphore::timedWait(const JeanUtility::CJeanTime& timeout) const
	{
...
namespace JeanUtility
{
	...
	_blocked(0),
	_unblocked(0),
	_state(JeanUtility::Cond::StateIdle)
	{
	}
	...
	void Cond::wake(bool broadcast)
	{
		...
		//
		JeanUtility::CJeanThreadMutex::Lock sync(_internal);
	...
	void Cond::postWait(bool timedOutOrFailed) const
	{
		CJeanThreadMutex::Lock sync(_internal);
	...
	bool Cond::timedDowait(const CJeanTime& timeout) const
	{
		try
		{
